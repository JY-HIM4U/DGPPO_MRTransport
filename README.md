...
| `dgppo/env/vmas_lidar/vmas_collaborative_transport_lidar_determined.py` | Fixed-initial-state variant used for deterministic evaluation (`test_determined.py`) |
...
| `train.py`, `test.py`, `test_determined.py` | Command-line flags for the above, plus evaluation and CSV/plot export |
| `pretrained/` | The policy behind the reported results, with its resolved config |

## License

DGPPO and this derived work are released under the license in
[LICENSE](LICENSE). Please respect the terms of the upstream project.

---

*The upstream DGPPO README follows.*
